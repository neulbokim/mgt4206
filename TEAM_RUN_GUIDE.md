...
python3 preprocessing/event_gbg.py
...
- `modeling/eda_covid_clean.ipynb`
...
- **EDA 시계열 그래프가 비정상일 때**
  ...
  - `modeling/eda_covid_clean.ipynb`를 위에서부터 다시 실행

- **SARIMAX에서 결측 관련 에러가 날 때**
  - `modeling/eda_covid_clean.ipynb`를 다시 실행해 master를 갱신
